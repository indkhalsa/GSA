...
int main()
{
	...
	cin>>it;
	double X[it][dm][in], fit[it][in], G[it], best[it], wrost[it], mg[it][in], mgall[it], Mg[it][in], Fij[it][dm][in][in-1], Fi[it][dm][in], A[it][dm][in], V[it][dm][in];
	...
	for(int t=1;t<=it;t++)
	{
		...
		for(int i=1;i<=in;i++)
		{
			double tempfit=4*X[t][1][i]*X[t][1][i]-2.1*X[t][1][i]*X[t][1][i]*X[t][1][i]*X[t][1][i]+1/3*X[t][1][i]*X[t][1][i]*X[t][1][i]*X[t][1][i]*X[t][1][i]*X[t][1][i]+X[t][1][i]*X[t][2][i]-4*X[t][2][i]*X[t][2][i]+4*X[t][2][i]*X[t][2][i]*X[t][2][i]*X[t][2][i];
			fit[t][i] = tempfit;
